...
async def create_space(display_name: str, space_type: str = 'SPACE', description: Optional[str] = None) -> Dict:
...
async def setup_space(display_name: str, members: List[str], description: Optional[str] = None) -> Dict:
    """Create a new space and add initial members in one call.

    Args:
        display_name: The display name for the new space
        members: List of user resource names to add (format: ['users/USER_ID', ...])
        description: Optional description for the space
    ...
    try:
        ...
        result = service.spaces().setup(body=body).execute()
        return result
    except Exception as e:
        raise Exception(f"Failed to setup space: {str(e)}")


async def update_space(space_name: str, display_name: Optional[str] = None, description: Optional[str] = None) -> Dict:
    """Update a space's display name and/or description.

    Args:
        space_name: The resource name of the space to update (format: 'spaces/SPACE_ID')
        display_name: New display name for the space. If None, not updated.
        description: New description for the space. If None, not updated.
    ...
    try:
        ...
        if description is not None:
            update_fields.append('spaceDetails.description')
            body['spaceDetails'] = {'description': description}

        if not update_fields:
            raise Exception("At least one of 'display_name' or 'description' must be provided")
...
async def add_member(space_name: str, user_id: str) -> Dict:
    ...
    try:
        ...
        result = service.spaces().members().create(
            parent=space_name,
            body={'member': {'name': user_id, 'type': 'HUMAN'}},
        ).execute()
        return result
    except Exception as e:
...
async def remove_member(member_name: str) -> Dict:
    ...
    try:
        ...
        return {'deleted': member_name, 'success': True}
    except Exception as e:
