...
@mcp.tool()
async def create_space(display_name: str, space_type: str = 'SPACE', description: str = None) -> Dict:
    """Create a new Google Chat space.

    Args:
        display_name: The display name for the new space
        space_type: Type of space — 'SPACE', 'GROUP_CHAT', or 'DIRECT_MESSAGE' (default: 'SPACE')
        description: Optional description for the space

    Returns:
        The created Space object with name, displayName, spaceType, and spaceDetails
    """
    from google_chat import create_space as _create_space
    return await _create_space(display_name, space_type, description)

@mcp.tool()
async def setup_space(display_name: str, members: list, description: str = None) -> Dict:
    """Create a new Google Chat space and add initial members in one call.

    More efficient than create_space + multiple add_member calls.

    Args:
        display_name: The display name for the new space
        members: List of user resource names to add as members (format: ['users/USER_ID', ...])
        description: Optional description for the space

    Returns:
        The created Space object with initial memberships
    """
    from google_chat import setup_space as _setup_space
    return await _setup_space(display_name, members, description)

@mcp.tool()
async def update_space(space_name: str, display_name: str = None, description: str = None) -> Dict:
    """Update a Google Chat space's display name and/or description.

    Args:
        space_name: The resource name of the space to update (format: 'spaces/SPACE_ID')
        display_name: New display name for the space. If not provided, not changed.
        description: New description for the space. If not provided, not changed.

    Returns:
        The updated Space object
    """
    from google_chat import update_space as _update_space
    return await _update_space(space_name, display_name, description)

@mcp.tool()
async def add_member(space_name: str, user_id: str) -> Dict:
    """Add a user to a Google Chat space.

    Args:
        space_name: The resource name of the space (format: 'spaces/SPACE_ID')
        user_id: The user resource name to add (format: 'users/USER_ID')

    Returns:
        The created Membership object with name, member, and role
    """
    from google_chat import add_member as _add_member
    return await _add_member(space_name, user_id)

@mcp.tool()
async def remove_member(member_name: str) -> Dict:
    """Remove a user from a Google Chat space.

    Use get_members() to find the full membership resource name.

    Args:
        member_name: The full membership resource name
                    (format: 'spaces/SPACE_ID/members/USER_ID')

    Returns:
        Confirmation of removal
    """
    from google_chat import remove_member as _remove_member
    return await _remove_member(member_name)
